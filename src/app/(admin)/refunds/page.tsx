  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div className="w-full max-w-5xl rounded-2xl border border-brand-purple/10 bg-white shadow-2xl">
          ...
          <div className="space-y-4 px-6 py-5">
            {refund.video_job.error_message && (
              <div className="rounded-2xl border border-red-200 bg-red-50 px-5 py-4">
                <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-red-600">Failure Reason</p>
                <p className="mt-2 text-[15px] font-medium leading-7 text-red-700">{refund.video_job.error_message}</p>
              </div>
            )}
            ...
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                <div key={label} className="rounded-xl border border-brand-purple/10 bg-white px-4 py-3">
                  <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-content-muted">{label}</p>
                  <p className="mt-1 text-sm font-semibold leading-6 text-content-primary">{value}</p>
                </div>
              ))}
            </div>
            ...
            {(refund.status === 'requested' || refund.status === 'approved' || refund.status === 'partial') && (
              <div className="rounded-2xl border border-brand-purple/10 bg-white p-4">
                ...
                <div className="mt-3 grid gap-4 md:grid-cols-3">
                  <div>
                    <label className="block text-xs font-semibold text-content-secondary">Approved Amount</label>
                    <input
                      type="number"
                      min="0"
                      step="0.01"
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-content-secondary">Payment Gateway</label>
                    <input
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-content-secondary">Payment Reference</label>
                    <input
            ...
            <div className="rounded-2xl border border-brand-purple/10 bg-surface-subtle/60 p-4">
              <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-content-muted">Customer Reason</p>
              <p className="mt-2 text-sm leading-7 text-content-secondary">{refund.reason}</p>
            </div>

            <div className="rounded-2xl border border-brand-purple/12 bg-white p-4">
              <p className="text-[11px] font-bold uppercase tracking-wide text-content-muted">Decision Note</p>
              <textarea
                value={note}
                onChange={(event) => setNote(event.target.value)}
                rows={4}
                className="mt-2 w-full rounded-xl border border-brand-purple/12 px-3 py-2.5 text-sm leading-6 text-content-primary outline-none focus:border-brand-purple"
